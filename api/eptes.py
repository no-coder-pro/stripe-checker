...
class PaymentMethodAdder:
    ...
    def extract_stripe_key_and_ajax_nonce(self):
        ...
        try:
            ...
            response = self.session.get(f'{self.base_url}/my-account-2/payment-methods/', headers=headers)
    ...
    def create_stripe_payment_method(self, card_data, hcaptcha_token=None):
        ...
        # Filter out empty values (especially muid/sid if not present)
...
def handle_endpoint(auth, hcaptcha_token='', email=None, password=None, proxy=None):
    ...
    try:
        ...
        payment_method_id, setup_intent_id, success, message = adder.run(card_data=card_data, hcaptcha_token=hcaptcha_token)
